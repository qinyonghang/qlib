...
NODISCARD FORCE_INLINE CONSTEXPR static auto align_up(T1 size, T2 alignment) noexcept {
...
public:
    ...
    template <class T>
    NODISCARD FORCE_INLINE CONSTEXPR static T* allocate(size_type n) noexcept {
        static_assert(sizeof(T) > 0, "cannot allocate zero-sized object");
        return (T*)(::operator new(n * sizeof(T)));
    }

    template <class T>
    FORCE_INLINE CONSTEXPR static void deallocate(T* p, size_type n ATTR_UNUSED) noexcept {
        ::operator delete(p
    ...
    FORCE_INLINE CONSTEXPR static void construct(T* p, Args&&... args) noexcept(
        is_nothrow_constructible_v<T, Args...>) {
        new (p) T(forward<Args>(args)...);
    }

    template <class T>
    FORCE_INLINE CONSTEXPR static void destroy(T* p) noexcept(is_nothrow_destructible_v<T>) {
        p->~T();
    }

    NODISCARD FORCE_INLINE CONSTEXPR static auto max_size() noexcept { return size_type(-1); }
};
...
public:
    FORCE_INLINE CONSTEXPR pool_allocator(size_type capacity = 64 * 1024) noexcept {
    ...
    template <class T>
    NODISCARD FORCE_INLINE CONSTEXPR T* allocate(size_type n) noexcept {
        static_assert(sizeof(T) > 0, "cannot allocate zero-sized object");
    ...
    template <class T>
    FORCE_INLINE CONSTEXPR void deallocate(T* p ATTR_UNUSED, size_type n ATTR_UNUSED) noexcept {}
};
...
public:
    ...
    template <class T>
    FORCE_INLINE CONSTEXPR void deallocate(T* p ATTR_UNUSED, uint64_t n ATTR_UNUSED) noexcept {}
};
...
public:
    FORCE_INLINE CONSTEXPR void operator()(value_type* __p) noexcept { delete __p; }
};
...
protected:
    ...
    NODISCARD FORCE_INLINE CONSTEXPR deleter_type& _deleter_() noexcept {
...
public:
    ...
    FORCE_INLINE CONSTEXPR unique_ptr() noexcept(is_nothrow_constructible_v<base>) = default;
    FORCE_INLINE CONSTEXPR
    unique_ptr(deleter_type& __d) noexcept(is_nothrow_constructible_v<base, deleter_type&>)
            : base(__d) {}

    FORCE_INLINE CONSTEXPR unique_ptr(self&& __o) noexcept(is_nothrow_constructible_v<base, base&&>)
            : base(move(__o)), _impl{__o._impl} {
        __o._impl = nullptr;
    }
    FORCE_INLINE CONSTEXPR explicit unique_ptr(value_type* __p) noexcept : _impl(__p) {}
    template <class... _Args>
    FORCE_INLINE CONSTEXPR explicit unique_ptr(value_type* __p, _Args&&... __args) noexcept(
        is_nothrow_constructible_v<base, deleter_type&>)
            : base(forward<_Args>(__args)...), _impl(__p) {}

    FORCE_INLINE ~unique_ptr() noexcept {
        if (_impl != nullptr) {
            _deleter_()(_impl);
        }
    }

    FORCE_INLINE CONSTEXPR void reset(value_type* __p = nullptr) noexcept {
        if (_impl != nullptr) {
            _deleter_()(_impl);
        }
        _impl = __p;
    ...
    FORCE_INLINE CONSTEXPR self& operator=(self&& __o) noexcept {
    ...
    FORCE_INLINE CONSTEXPR self& operator=(value_type* __p) noexcept {
        reset(__p);
        return *this;
    }

    template <class _Up = value_type>
    NODISCARD FORCE_INLINE CONSTEXPR enable_if_t<!is_same_v<_Up, void>, _Up&> operator*() noexcept {
        return *_impl;
    }

    template <class _Up = value_type>
    NODISCARD FORCE_INLINE CONSTEXPR enable_if_t<!is_same_v<_Up, void>, _Up const&> operator*()
        const noexcept {
        return *_impl;
    }

    NODISCARD FORCE_INLINE CONSTEXPR value_type* operator->() noexcept { return _impl; }
    NODISCARD FORCE_INLINE CONSTEXPR value_type const* operator->() const noexcept { return _impl; }

    NODISCARD FORCE_INLINE CONSTEXPR value_type* get() noexcept { return _impl; }
    NODISCARD FORCE_INLINE CONSTEXPR value_type const* get() const noexcept { return _impl; }

    NODISCARD FORCE_INLINE CONSTEXPR bool_t empty() const noexcept { return _impl == nullptr; }
    NODISCARD FORCE_INLINE CONSTEXPR explicit operator bool_t() const noexcept { return empty(); }
    NODISCARD FORCE_INLINE CONSTEXPR bool_t operator==(nullptr_t) const noexcept { return empty(); }
    NODISCARD FORCE_INLINE CONSTEXPR bool_t operator!=(nullptr_t) const noexcept {
        return !empty();
    }
};
...
NODISCARD FORCE_INLINE CONSTEXPR auto make_unique(_Args&&... args) {
